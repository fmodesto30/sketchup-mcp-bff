...
function Chip({ on, label }: { on?: boolean; label: string }) {
  return (
    <span className="inline-flex items-center gap-1.5 rounded-full border border-white/10 bg-white/[0.04] px-2.5 py-1 text-xs text-white/70">
      <span className={cn("size-1.5 rounded-full", on ? "bg-ok shadow-[0_0_6px] shadow-ok" : "bg-white/30")} />
      {label}
    </span>
  );
}
...
export function FlowHero({ onSeeFlow }: { onSeeFlow: () => void }) {
  ...
  return (
    <motion.div
      ...
      className="relative overflow-hidden rounded-xl border border-border bg-gradient-to-br from-[#11111c] via-[#0c0d12] to-[#0a0b0e] p-7 sm:p-10"
    >
      ...
      <div className="relative">
        <div className="mb-4 flex flex-wrap items-center gap-2">
          ...
          <span className="rounded-full border border-white/10 px-3 py-1 text-xs font-medium text-white/60">AI Cockpit</span>
          <span className="rounded-full border border-white/10 px-3 py-1 text-xs font-medium text-white/60">Documentação viva</span>
        </div>

        <h1 className="max-w-3xl text-3xl font-bold leading-[1.1] tracking-tight text-white sm:text-[2.6rem]">
          Da planta <span className="bg-gradient-to-r from-primary to-primary/60 bg-clip-text text-transparent">PDF</span> ao{" "}
          <span className="bg-gradient-to-r from-white to-primary bg-clip-text text-transparent">.skp</span> validado, renderizado e aprovado
        </h1>
